...
namespace FastColoredTextBoxNS
{
    ...
    public class Hints : ICollection<Hint>, IDisposable
    {
        ...
        public List<Hint> items = new List<Hint>();
        ...
        public void Refresh()
        {
            Hint[] oldHints = new Hint[items.Count];
            items.CopyTo(oldHints);
            tb.ClearHints();
            foreach (var hint in oldHints)
                tb.AddHint(hint.Range.tb, hint.Suggestion, false, true, true);
        }
    }
    ...
    public class Hint
    {
        ...
        /// <param name="inline">Inlining. If True then hint will moves apart text</param>
        /// <param name="dock">Docking. If True then hint will fill whole line</param>
        public Hint(Range range, Suggestion suggestion, bool inline, bool dock)
            : this(range, null, suggestion.message, inline, dock)
        {
        ...
        {
            var tb = this.Range.tb;
            if (this.Suggestion.hasSuggestion)
            {
                string newText = this.Suggestion.suggestion;
                ...
                if (tb.Selection.ColumnSelectionMode)
                {
                    ...
                    start.iChar = this.Range.Start.iChar;
                    end.iChar = this.Range.End.iChar;
                    tb.Selection.Start = start;
                    tb.Selection.End = end;
                }
                else
                {
                    tb.Selection.Start = this.Range.Start;
                    tb.Selection.End = this.Range.End;
                }
        ...
        protected virtual void Init()
        {
            ...
            BackColor = InnerControl == null ? Color.Gold : SystemColors.Control;
