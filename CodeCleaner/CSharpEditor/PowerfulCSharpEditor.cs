...
namespace CSharpEditor
{
    public partial class PowerfulCSharpEditor : Form
    {
                ...
                if (e.ColumnIndex == 1)
                    ...
                    switch (item.type)
                    {
                        case ExplorerItemType.Class:
                            e.Value = global::CSharpEditor.Properties.Resources.class_libraries;
                            return;
                        case ExplorerItemType.Method:
                            e.Value = global::CSharpEditor.Properties.Resources.box;
                            return;
                        case ExplorerItemType.Event:
                            e.Value = global::CSharpEditor.Properties.Resources.lightning;
                            return;
                        case ExplorerItemType.Property:
                            e.Value = global::CSharpEditor.Properties.Resources.property;
                            return;
                    }
            }
            catch {; }
        ...
        {
            if (tsFiles.SelectedItem != null && CurrentTB != null)
            {
                if (CurrentTB.IsChanged)
                    switch (MessageBox.Show("Needs current tab to be saved.\nDo you want save " + tsFiles.SelectedItem.Title + " ?",
                ...
                Save(tsFiles.SelectedItem);
                Scanner scanner = new Scanner(tsFiles.SelectedItem.Tag.ToString());
                Parser parser = new Parser(scanner);
                parser.Parse();
                for (int i = parser.cleaner.suggestions.Count - 1; i >= 0; i--)
                {
                    var suggestion = parser.cleaner.suggestions[i];
                    CurrentTB.AddHint(CurrentTB, suggestion, false, true, true);
                }
                MessageBox.Show("Total suggestion number: " + parser.cleaner.suggestions.Count.ToString(),
                    "Code Cleaner", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
                MessageBox.Show("No selected tab!", "Code Cleaner", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
